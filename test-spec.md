...
| 3-2  | 全API共通 | `text`フィールドなし | `text`を含まないJSONをPOST | 400エラーと`{"error": "Missing 'text' in request body"}`が返る。 |
| 3-3  | 全API共通 | `text`が空文字 | `{"text": " "}`をPOST | 400エラーと`{"error": "Input text cannot be empty"}`が返る。 |
...
| 3-9  | 全API共通 | 日本語以外の入力 | `{"text": "hello"}` のように日本語を含まない文字列をPOST | 400エラーと`{"error": "日本語で入力してください。"}`が返る。 |
...
| 5-8  | プロット・名前生成（日本語以外） | 各入力欄に "hello" と入力して生成ボタンをクリック | 「日本語で入力してください。」というエラーメッセージが表示される。 |
...
| 6-6  | 類語・描写生成（日本語以外） | 各入力欄に "hello" と入力して生成ボタンをクリック | 「日本語で入力してください。」というエラーメッセージが表示される。 |
...
| No | テスト内容 | 手順 | 期待結果 |
|----|------------|------|----------|
| 8-1  | レスポンス速度の計測 | ユーザーが入力してからAIが出力を返すまでの時間を測る。 | プロット出力は約13、14秒程度。 名前生成は約8秒程度。類語検索は約8、15秒程度。描写の具体化は8秒程度。|
